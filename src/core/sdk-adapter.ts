...
export class SdkAcpClientAdapter implements AcpClientAdapter {
  ...
  async connect({ handlers, signal }: AcpAdapterConnectOptions): Promise<AcpClientConnection> {
    let app = client({ name: this.name })
      .onNotification(methods.client.session.update, ({ params }) => handlers.sessionUpdate(params))
      .onRequest(methods.client.session.requestPermission, ({ params, signal: requestSignal }) =>
        handlers.requestPermission(params, requestSignal),
      );

    if (handlers.readTextFile) {
      app = app.onRequest(methods.client.fs.readTextFile, ({ params, signal: requestSignal }) =>
        handlers.readTextFile!(params, requestSignal),
      );
    }
    if (handlers.writeTextFile) {
      app = app.onRequest(methods.client.fs.writeTextFile, ({ params, signal: requestSignal }) =>
    ...
    if (handlers.terminal) {
      ...
      app = app
        .onRequest(methods.client.terminal.create, ({ params, signal: requestSignal }) =>
          terminal.create(params, requestSignal),
        )
        .onRequest(methods.client.terminal.output, ({ params, signal: requestSignal }) =>
          terminal.output(params, requestSignal),
        )
        .onRequest(methods.client.terminal.release, ({ params, signal: requestSignal }) =>
          terminal.release(params, requestSignal),
        )
        .onRequest(methods.client.terminal.waitForExit, ({ params, signal: requestSignal }) =>
          terminal.waitForExit(params, requestSignal),
        )
        .onRequest(methods.client.terminal.kill, ({ params, signal: requestSignal }) =>
          terminal.kill(params, requestSignal),
        );
    }
