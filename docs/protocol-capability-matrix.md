...
状态含义：`实现`为直接支持；`能力门控`为仅在 initialize 声明后开放；`可选扩展`为支持时使用、缺失时兼容回退；`不适用`为协议没有对应语义且不会伪造。
...
| `authentication/status`                       | 可选扩展       | SDK adapter、controller                     | `controller.test.ts`、`stream-conformance.test.ts` |
